...
import com.zhengzangw.gesturedetect.dialogs.DialogResultListener;
import com.zhengzangw.gesturedetect.dialogs.PromptDialog;
import com.zhengzangw.gesturedetect.utils.AppSettings;
import com.zhengzangw.gesturedetect.utils.ColorsAdapter;
import com.zhengzangw.gesturedetect.utils.Label;
...
public class MainActivity extends AppCompatActivity implements DialogResultListener {
    ...
    private void initViews() {
        ...
        txtStats = findViewById(R.id.txtState);
        spinLabels = findViewById(R.id.spinLabels);

        btnRec.setOnClickListener(clickListener);
        ...
        yRightAxis.setTextColor(Color.BLACK);
        yRightAxis.setAxisMaximum((10f));
        yRightAxis.setAxisMinimum((-10f));
    ...
    //onResume -> onRequestPermissionResult
    ...
    //Begin: util function for onCreate
    ...
    private String formatStatsText() {
        return String.format(Locale.getDefault(), "Pos: %s/%s s\nSamples: %d", getXLabelAtHighlight(), getXLabelAtEnd(), getSamplesCount());
    }

    private String getXLabelAtHighlight() {
        if ((selectedEntryIndex == -1) || (getLineData().getDataSetCount() == 0)) return "-";
        return String.format(Locale.getDefault(), "%.03f", getLineData().getDataSetByIndex(0).getEntryForIndex(selectedEntryIndex).getX() / 1000f);
    }

    private String getXLabelAtEnd() {
        if ((getLineData().getDataSetCount() == 0) || (getLineData().getDataSetByIndex(0).getEntryCount() == 0))
            return "-";
        return String.format(Locale.getDefault(), "%.03f", getLineData().getDataSetByIndex(0).getEntryForIndex(getLineData().getDataSetByIndex(0).getEntryCount() - 1).getX() / 1000f);
    }
    ...
    private void launchDemoActivity() { startActivity(new Intent(this, DemoActivity.class)); }
